...
<div class="x-body">
    ...
    <xblock>
        <button class="layui-btn layui-btn-danger" data-btn="deleteAll" data-action="<?php echo ($disable_admin_href); ?>">
            <i class="layui-icon"></i>批量删除
        </button>
        <button class="layui-btn" onclick="x_admin_show('<?php echo ($add_admin_page_title); ?>','<?php echo ($add_admin_page_href); ?>',600,300)">
        ...
        <?php for($i=0; $i<count($result); $i++) { ?>
            <tr id="content">
                ...
                <td class="td-status">
                    ...
                    <span class="layui-btn layui-btn-normal layui-btn-mini <?php echo ($bgState); ?>">
                        <!-- 状态显示操作 -->
                        <?php if($result[$i][state]=='1') {echo'已启用';}else{echo '已禁用';} ?>
                    </span>
                </td>
                <td class="td-manage">
                    <a id="edit" class="send" title="编辑" onclick="show(this)" href="javascript:void(0)" data-btn="edit" data-user="<?php echo ($result[$i][id]); ?>" data-pagehref="<?php echo ($edit_admin_page_href); ?>" data-username="<?php echo ($result[$i][username]); ?>">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a id="disable" title="禁用" onclick="disable(this)" href="javascript:void(0)" data-user="<?php echo ($result[$i][id]); ?>" data-pagehref="<?php echo ($disable_admin_href); ?>" data-username="<?php echo ($result[$i][username]); ?>">
                        <i class="layui-icon">&#x1006;</i>
                    </a>
                    <a title="删除" onclick="del(this)" href="javascript:void(0)" data-btn="delete" data-user="<?php echo ($result[$i][id]); ?>" data-pagehref="<?php echo ($disable_admin_href); ?>" data-username="<?php echo ($result[$i][username]); ?>">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
